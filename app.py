...
st.write('---')
with st.popover('Przedstaw się', use_container_width=True):
    name = st.text_input('Witaj! Jak masz na imię?')
st.markdown(f'## Cześć {name}! :hand:')
...
with st.expander('Opisz siebie', expanded=False):
    ...
    user_data = st.text_area('Tutaj wpisz informacje:')

if user_data:

    data_for_predict = retrieve_structure_observed(user_data, User)
    ...
    if missing_columns:
        ...
        st.success(f'Estymowany czas ukończenia półmaratonu (w formacie: H:M:S) wynosi {prediction_time}')
        #st.write(data_for_predict)
